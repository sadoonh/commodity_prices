...
smoothed_train_predict = comparison_df['Train Predict'].rolling(window=10).mean()
smoothed_test_predict = comparison_df['Test Predict'].rolling(window=10).mean()
...
layout = go.Layout(
    title=f'Actual vs Predicted {commodity} Prices (Last 4 Months, Smoothed)',
    xaxis_title='Date',
